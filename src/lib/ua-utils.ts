...
interface PlatformInfo { platform: string }
interface UserAgentData { browser: BrowserInfo; os: OSInfo; platform: PlatformInfo }
...
function getOSVersion(userAgent: string): string {
  ...
  if (winMatch) {
    const versionMap: Record<string, string> = {
      ...
      '6.0': 'Vista', '5.2': 'Server 2003/XP x64', '5.1': 'XP', '5.0': '2000'
    };
  ...
  if (macMatch) {
    ...
    const macVersionMap: Record<string, string> = {
      ...
      '10.11': 'El Capitan', '10.10': 'Yosemite', '10.9': 'Mavericks',
...
function getUserAgentData(userAgent: string): UserAgentData {
  return {
    browser: { name: getBrowserName(userAgent), version: getBrowserVersion(userAgent) },
